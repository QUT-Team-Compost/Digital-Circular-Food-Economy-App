...
function App(props) {
  ...
  if (!ready) {
    ...
    // Use AppLoading to display the splash screen, and wait for the assets to
  ...
  // If the app is ready...
    ...
    if (PASSCODE_HASH_ACTUAL && !bcrypt.compareSync(passCode, accessPassCode) && !bcrypt.compareSync(passCode, accessPassCodeTest)) {
      ...
      // Show a screen asking the user to enter a passcode, including a text
      // box for them to do so.
      return (
      <>
  ...
  return (
    ...
    /* Put the app in a navigation container to allow for navigating between
    screens.*/
    <NavigationContainer ref={navigationRef}>
      ...
      <View>
        ...
        {/* Create a horizontal scroll view for the menu and generate menu
        items from the route list. */}
        ...
        {/* Create a header for the screen, taking the corresponding screen
        name from the route list. */}
        <View style={sharedStyles.pageHeader}><Text style={sharedStyles.pageHeaderText}>{routeIdList.filter(obj => { return obj.id === menu.currentPage })[0].name}</Text></View>
      </View>
